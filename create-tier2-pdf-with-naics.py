#!/usr/bin/env python3
"""
Create Tier 2 PDF with NAICS codes and descriptions
"""
...
# Read contacts
csv_file = '/Users/ericcoffie/Bootcamp/TIER-2-FINAL-CONTACT-LIST-CLEAN.csv'
contacts = []
...
phone_count = sum(1 for c in contacts if c.get('phone'))
both_count = sum(1 for c in contacts if c.get('email') and c.get('phone'))
...
for contact in contacts:
    ...
    sblo_name = html.escape(contact.get('sblo_name', ''))
    email = html.escape(contact.get('email', ''))
    phone = html.escape(contact.get('phone', ''))
    naics_formatted = format_naics_list(contact.get('naics', ''))
...
<head>
    ...
    <title>Tier 2 Subcontractor Contact Directory with NAICS</title>
    <style>
        @page {{
...
<div class="header">
    <h1>Tier 2 Subcontractor Contact Directory</h1>
    <div class="subtitle">Mid-Tier Federal Contractor Contacts with Industry Focus (NAICS)</div>
...
<div class="footer">
    <p><strong>Tier 2 Subcontractor Directory with Industry Focus</strong> | December 2025</p>
    <p style="margin-top: 5px;">
        Sources: DHS Prime Contractors, SBA Prime Directory FY24, DoD CSP Directory, DOT Subcontracting Directory
    </p>
    <p style="margin-top: 8px; font-style: italic;">
        Generated with <a href="https://claude.com/claude-code" style="color: #047857; text-decoration: none;">Claude Code</a>
    </p>
</div>
...
# Write output
output_file = '/Users/ericcoffie/Bootcamp/TIER-2-DIRECTORY-WITH-NAICS-PDF-READY.html'
with open(output_file, 'w', encoding='utf-8') as f:
    f.write(html_content)

print("=" * 80)
print("TIER 2 PDF WITH NAICS CREATED")
print("=" * 80)
print(f"\n✅ Created Tier 2 directory with NAICS industry codes")
print(f"📄 File: {output_file}")
print(f"\n📊 Statistics:")
print(f"   • {len(contacts)} companies (Tier 1 companies removed)")
print(f"   • {email_count} with email")
print(f"   • {phone_count} with phone")
print(f"   • {both_count} with both")
print(f"\n📝 TO CREATE PDF:")
print(f"   1. Open file in browser")
